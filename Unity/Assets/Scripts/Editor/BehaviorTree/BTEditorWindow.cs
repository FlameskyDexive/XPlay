...
namespace ET
{
    public sealed class BTEditorWindow : EditorWindow
    {
        ...
        public static void Open(BTAsset asset = null)
        {
            BTEditorWindow window = GetWindow<BTEditorWindow>(DockDefine.Types);
            window.titleContent = new GUIContent("BehaviourTreeEditor");
            BTAsset selectedAsset = asset ?? GetSelectedAsset();
        ...
        {
            if (EditorUtility.InstanceIDToObject(instanceId) is not BTAsset asset)
            {
                return false;
            }
        ...
        private static BTAsset GetSelectedAsset()
        {
            if (Selection.activeObject is BTAsset behaviorTreeAsset)
            {
            ...
            {
                if (selectedObject is BTAsset asset)
                {
                    return asset;
                }
            }
        ...
        public void OpenNodeScript(BTNodeView nodeView)
        {
            ...
            if (BTEditorUtility.TryOpenNodeScript(nodeView.Data))
            {
                return;
            }
        ...
        private void OnEnable()
        {
            ...
            this.connectionStyle = (BTConnectionStyle)EditorPrefs.GetInt(ConnectionStyleEditorPrefKey, (int)BTConnectionStyle.Orthogonal);
        }
        ...
        private void CreateGUI()
        {
            ...
            try
            {
                this.graphView = new BTGraphView(this);
            }
            catch (Exception exception)
            {
                Label errorLabel = new($"BTGraphView init failed:\n{exception}");
            ...
            if (this.asset == null && Selection.activeObject is BTAsset selectedAsset)
            {
        ...
        {
            if (Selection.activeObject is BTAsset selectedAsset)
            {
                this.LoadAsset(selectedAsset);
            }
        }
        ...
        private Toolbar CreateToolbar()
        {
            ...
            lineMenu.menu.AppendAction("Straight", _ => this.SetConnectionStyle(BTConnectionStyle.Straight),
                _ => this.connectionStyle == BTConnectionStyle.Straight ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
            lineMenu.menu.AppendAction("Orthogonal", _ => this.SetConnectionStyle(BTConnectionStyle.Orthogonal),
                _ => this.connectionStyle == BTConnectionStyle.Orthogonal ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
            lineMenu.menu.AppendAction("Curve", _ => this.SetConnectionStyle(BTConnectionStyle.Curve),
                _ => this.connectionStyle == BTConnectionStyle.Curve ? DropdownMenuAction.Status.Checked : DropdownMenuAction.Status.Normal);
        ...
        private void SetConnectionStyle(BTConnectionStyle style)
        {
        ...
        private string GetConnectionStyleLabel()
        {
            ...
            {
                BTConnectionStyle.Straight => "Line: Straight",
                BTConnectionStyle.Curve => "Line: Curve",
                _ => "Line: Orthogonal",
            };
        }
        ...
        private void LoadAsset(BTAsset newAsset)
        {
            this.asset = newAsset;
        ...
        private ToolbarMenu CreateAssetMenu()
        {
            ...
            string[] guids = AssetDatabase.FindAssets("t:BTAsset");
            foreach (string guid in guids)
            {
                string path = AssetDatabase.GUIDToAssetPath(guid);
                BTAsset behaviorTreeAsset = AssetDatabase.LoadAssetAtPath<BTAsset>(path);
        ...
        private void DrawInspectorPanel()
        {
            ...
            BTAsset selectedAsset = (BTAsset)EditorGUILayout.ObjectField("Asset", this.asset, typeof(BTAsset), false);
        ...
        private void DrawNodeInspector(BTEditorNodeData node)
        {
            BTEditorUtility.SyncNodeDescriptor(node);
            EditorGUILayout.LabelField("Script", BTEditorUtility.GetNodeScriptName(node));
            ...
            switch (node.NodeKind)
            {
                case BTNodeKind.Action:
                    if (BTEditorUtility.TryGetDescriptor(node, out ABTNodeDescriptor actionDescriptor))
                    {
                    ...
                    {
                        DrawHandlerPopup("Handler", BTEditorUtility.GetActionHandlerNames(), node, value => node.HandlerName = value);
                        DrawArguments(node);
                    }
                    break;
                case BTNodeKind.Condition:
                    if (BTEditorUtility.TryGetDescriptor(node, out ABTNodeDescriptor conditionDescriptor))
                    {
                        this.DrawDescriptorNode(node, conditionDescriptor);
                    }
                    else
                    {
                        DrawHandlerPopup("Handler", BTEditorUtility.GetConditionHandlerNames(), node, value => node.HandlerName = value);
                        DrawArguments(node);
                    }
                    break;
                case BTNodeKind.Service:
                    if (BTEditorUtility.TryGetDescriptor(node, out ABTNodeDescriptor serviceDescriptor))
                    {
                        this.DrawDescriptorNode(node, serviceDescriptor, true);
                    }
                    else
                    {
                        DrawHandlerPopup("Handler", BTEditorUtility.GetServiceHandlerNames(), node, value => node.HandlerName = value);
                ...
                case BTNodeKind.Repeater:
                    ...
                    break;
                case BTNodeKind.BlackboardCondition:
                    DrawBlackboardKeyPopup(node);
                    node.CompareOperator = (BTCompareOperator)EditorGUILayout.EnumPopup("Operator", node.CompareOperator);
                    node.AbortMode = (BTAbortMode)EditorGUILayout.EnumFlagsField("Abort Mode", node.AbortMode);
                    DrawSerializedValueEditor("Compare Value", node.CompareValue);
                    break;
                case BTNodeKind.Parallel:
                    node.SuccessPolicy = (BTParallelPolicy)EditorGUILayout.EnumPopup("Success Policy", node.SuccessPolicy);
                    node.FailurePolicy = (BTParallelPolicy)EditorGUILayout.EnumPopup("Failure Policy", node.FailurePolicy);
                    break;
                case BTNodeKind.SubTree:
                    node.SubTreeAsset = (BTAsset)EditorGUILayout.ObjectField("SubTree", node.SubTreeAsset, typeof(BTAsset), false);
        ...
        private void DrawBlackboardPanel()
        {
            ...
            {
                BTBlackboardEntryData entry = this.asset.BlackboardEntries[index];
                ...
                entry.ValueType = (BTValueType)EditorGUILayout.EnumPopup("Type", entry.ValueType);
            ...
            {
                this.asset.BlackboardEntries.Add(new BTBlackboardEntryData());
            }
        ...
        private static object DrawRuntimeValueField(BTValueType valueType, object currentValue)
        {
            switch (valueType)
            {
                case BTValueType.Integer:
                    return EditorGUILayout.IntField("Value", currentValue is int intValue ? intValue : 0);
                case BTValueType.Long:
        ...
        private void DrawBlackboardKeyPopup(BTEditorNodeData node)
        {
        ...
        private void ExportCurrentAsset()
        {
            ...
            try
            {
                BTExporter.BTExportResult result = BTExporter.ExportToFiles(this.asset);
        ...
        private void DrawPatrolPointsEditor(BTEditorNodeData node)
        {
            EditorGUILayout.Space(6);
            EditorGUILayout.LabelField("Patrol Points", EditorStyles.boldLabel);
            for (int index = 0; index < node.PatrolPoints.Count; ++index)
            {
                ET.BTPatrolPointData patrolPoint = node.PatrolPoints[index] ?? new ET.BTPatrolPointData();
            ...
            {
                node.PatrolPoints.Add(new ET.BTPatrolPointData());
            }
        }

        private void DrawDescriptorArguments(BTEditorNodeData node, IReadOnlyList<BTNodeParameterDefinition> parameters)
        {
        ...
        private void DrawDescriptorArgument(BTEditorNodeData node, BTNodeParameterDefinition parameter)
        {
            BTArgumentData argument = BTEditorUtility.GetOrCreateArgument(node, parameter);
            ...
            {
                case BTNodeParameterEditorHint.BlackboardKey:
                    this.DrawBlackboardKeyArgument(label, argument);
                    break;
                case BTNodeParameterEditorHint.CompareOperator:
                    argument.Value.ValueType = BTValueType.Integer;
                    BTCompareOperator compareOperator = (BTCompareOperator)argument.Value.IntValue;
                    compareOperator = (BTCompareOperator)EditorGUILayout.EnumPopup(label, compareOperator);
                    argument.Value.IntValue = (int)compareOperator;
                    break;
                case BTNodeParameterEditorHint.MultilineText:
                    argument.Value.ValueType = BTValueType.String;
        ...
        private void DrawBlackboardKeyArgument(string label, BTArgumentData argument)
        {
            argument.Value ??= new BTSerializedValue();
            argument.Value.ValueType = BTValueType.String;
            string[] options = this.GetBlackboardKeyOptions();
            if (options.Length == 0)
            {
        ...
        private static void DrawArguments(BTEditorNodeData node)
        {
            ...
            {
                node.Arguments.Add(new BTArgumentData());
            }
        }

        private static void DrawHandlerPopup(string label, string[] options, BTEditorNodeData node, Action<string> assign)
        {
        ...
        private static void DrawSerializedValueEditor(string label, BTSerializedValue value, BTValueType forceType = BTValueType.None)
        {
            if (value == null)
            {
                return;
            ...
            value.ValueType = forceType == BTValueType.None
                    ? (BTValueType)EditorGUILayout.EnumPopup(label + " Type", value.ValueType)
                    : forceType;
