...
namespace ET
{
    public sealed class BTGraphView : GraphView
    {
        ...
        {
            public readonly List<BTEditorNodeData> Nodes = new();
            public readonly List<(string ParentId, string ChildId)> Connections = new();
        }

        private static ClipboardSelection clipboard;
        private readonly BTConnectionLayer connectionLayer;
        private readonly Dictionary<string, BTNodeView> nodeViews = new();
        private readonly BTEditorWindow window;
        private readonly BTGridBackground gridBackground;
        private readonly BTSearchWindowProvider searchWindowProvider;
        private bool isPopulating;
        private BTMiniMap miniMap;
        ...
        public BTGraphView(BTEditorWindow window)
        {
            ...
            this.gridBackground = new BTGridBackground(this);
            ...
            this.connectionLayer = new BTConnectionLayer(this);
            ...
            this.searchWindowProvider = ScriptableObject.CreateInstance<BTSearchWindowProvider>();
        ...
        public BTAsset Asset { get; private set; }

        public BTEditorWindow GetWindow()
        {
            return this.window;
        }
        ...
        public void PopulateView(BTAsset asset)
        {
            ...
            foreach (BTEditorNodeData node in asset.Nodes)
            {
                if (!this.nodeViews.TryGetValue(node.NodeId, out BTNodeView parentView) || parentView.OutputPort == null)
                {
                    continue;
                }
                ...
                {
                    if (!this.nodeViews.TryGetValue(childId, out BTNodeView childView) || childView.InputPort == null)
                    {
                        continue;
                    }
        ...
        public void CreateNode(BTNodeKind nodeKind, Vector2 localMousePosition, string nodeTypeId = "")
        {
            if (this.Asset == null)
            {
                return;
        ...
        public void CreateNodeAtContentPosition(BTNodeKind nodeKind, Vector2 contentPosition, string nodeTypeId = "")
        {
            if (this.Asset == null)
            {
                return;
            ...
            BTEditorNodeData node = this.Asset.AddNode(nodeKind, contentPosition, nodeTypeId);
            this.AddNodeView(node);
        ...
        public void PasteNodes(Vector2? centerPosition = null)
        {
            ...
            foreach (BTEditorNodeData clipboardNode in clipboard.Nodes)
            {
            ...
            Dictionary<string, BTEditorNodeData> pastedNodes = new();
            foreach (BTEditorNodeData clipboardNode in clipboard.Nodes)
            {
                BTEditorNodeData pastedNode = clipboardNode.Clone();
            ...
            {
                if (!pastedNodes.TryGetValue(parentId, out BTEditorNodeData parentNode) || !pastedNodes.TryGetValue(childId, out BTEditorNodeData childNode))
                {
                    continue;
                }
            ...
            this.ClearSelection();
            foreach (BTEditorNodeData pastedNode in pastedNodes.Values)
            {
                if (this.nodeViews.TryGetValue(pastedNode.NodeId, out BTNodeView pastedView))
                {
                    this.AddToSelection(pastedView);
                }
            }

            if (pastedNodes.Values.FirstOrDefault() is BTEditorNodeData firstPastedNode && this.nodeViews.TryGetValue(firstPastedNode.NodeId, out BTNodeView firstPastedView))
            {
        ...
        public void AutoLayoutTree()
        {
            ...
            float Measure(string nodeId)
            {
                ...
                BTEditorNodeData node = this.Asset.GetNode(nodeId);
            ...
            foreach (BTEditorNodeData node in this.Asset.Nodes)
            {
                if (connectedNodeIds.Contains(node.NodeId) || node.NodeKind == BTNodeKind.Root)
                {
                    continue;
                }
        ...
        {
            List<BTNodeView> selectedNodeViews = this.GetSelectedNodeViews();
        ...
        {
            List<BTNodeView> selectedNodeViews = this.GetSelectedNodeViews().OrderBy(view => view.GetPosition().center.x).ToList();
        ...
        {
            List<BTNodeView> selectedNodeViews = this.GetSelectedNodeViews().OrderBy(view => view.GetPosition().center.y).ToList();
            if (selectedNodeViews.Count < 3)
            {
                return;
            }
        ...
        public BTConnectionStyle GetConnectionStyle()
        {
        ...
        public void SelectNode(string nodeId)
        {
            ...
            if (!this.nodeViews.TryGetValue(nodeId, out BTNodeView nodeView))
            {
                return;
            }
        ...
        public void RefreshDebugStates(BTDebugSnapshot snapshot)
        {
            foreach (BTNodeView nodeView in this.nodeViews.Values)
            {
                BTNodeState state = BTNodeState.Inactive;
                if (snapshot != null && snapshot.NodeStates.TryGetValue(nodeView.Data.NodeId, out BTNodeState debugState))
                {
                    state = debugState;
                }
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            BTNodeView contextNodeView = this.GetContextNodeView(evt);
        ...
        private void AddNodeView(BTEditorNodeData node)
        {
            BTNodeView view = new(node, this.window.SelectNode, () =>
            {
        ...
        private void OpenNodeScript(BTNodeView nodeView)
        {
            if (nodeView == null)
            {
                return;
        ...
        private bool CanOpenNodeScript(BTNodeView nodeView)
        {
            return nodeView != null && BTEditorUtility.TryGetNodeScriptAsset(nodeView.Data, out _);
        }

        private BTNodeView GetContextNodeView(ContextualMenuPopulateEvent evt)
        {
            if (evt?.target is BTNodeView directNodeView)
            {
            ...
            {
                BTNodeView ancestorNodeView = visualElement.GetFirstAncestorOfType<BTNodeView>();
        ...
        {
            List<BTNodeView> selectedNodeViews = this.selection.OfType<BTNodeView>().Where(view => BTEditorUtility.CanDelete(view.Data)).ToList();
            ...
            foreach (BTNodeView nodeView in selectedNodeViews)
            {
        ...
        private void ApplyNodePositions(List<BTNodeView> nodeViewsToMove, Func<BTNodeView, Rect> positionFactory, string undoName)
        {
            ...
            foreach (BTNodeView nodeView in nodeViewsToMove)
            {
        ...
        {
            return this.selection.OfType<BTNodeView>().ToList();
        }

        private void DeleteSelectionNodes()
        {
            List<GraphElement> elements = this.selection
                .Where(element => element is BTNodeView nodeView ? BTEditorUtility.CanDelete(nodeView.Data) : element is Edge)
                .Cast<GraphElement>()
                .ToList();
        ...
        {
            return this.selection.OfType<BTNodeView>().Any(view => BTEditorUtility.CanDelete(view.Data));
        }
        ...
        {
            return this.selection.Any(element => element is Edge || element is BTNodeView nodeView && BTEditorUtility.CanDelete(nodeView.Data));
        }
        ...
        private void EnsureMiniMap()
        {
            ...
            this.miniMap = new BTMiniMap(this);
        ...
        {
            if (edge.output?.node is not BTNodeView parentView || edge.input?.node is not BTNodeView childView)
            {
                return;
            }
        ...
        public IEnumerable<BTNodeView> GetNodeViews()
        {
